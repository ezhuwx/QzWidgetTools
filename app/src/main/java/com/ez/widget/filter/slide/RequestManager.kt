...
class RequestManager(
     ...
     * 本地数据
     */
    var localData: MutableList<DictData>? = null
            ...
            dataList?.let { data ->
                ...
                if (isChildRequest) {
                    ...
                    if (configuration.isHadChildren) parseData.forEach { parent ->
                        configuration.parentChildMap[parent.realId(configuration.isNameCodeMode)] =
                            arrayListOf()
                    }
                }
     ...
     * 数据配置
     */
    private fun onBuildData(dataList: MutableList<DictData>?, parentId: String?, isChild: Boolean) {
        onBuildDataListener?.onBuildData(dataList, parentId, isChild)
    }

    fun isNameCodeMode(isNameCodeMode: Boolean = true): RequestManager {
        configuration.isNameCodeMode = isNameCodeMode
        return this
    }
    ...
    fun interface OnBuildDataListener {
        fun onBuildData(dataList: MutableList<DictData>?, parentId: String?, isChild: Boolean)
    }
    ...
    fun interface OnRequestDataListener {
        suspend fun onRequestData(
            ...
            parentId: String?,
            isChild: Boolean
        ): Pair<MutableList<DictData>?, Boolean>
    }
}
